    ...
    public Credentials(String driverUrl, Properties info) throws SQLException {
        ...
        if (user == null || password == null) {
            throw new SQLException("user or password missing; URL must be of form"
                    + " 1) \"jdbc:kr:kintone:myUser:myPassword\" where \"user\" and \"password\" are defined in the url"
                    + " or"
                    + " 2) \"jdbc:kr:kintone[(;[name=value])+]\" where \"user\" and \"password\" are defined in the url"
                    + " (e.g. \"jdbc:kr:kintone;user=myUser;password=myPassword\") or in the connection properties"
                    );
        }
    }
